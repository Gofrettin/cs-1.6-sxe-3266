...
bool FakeEdge(float& angle)
{
	...
	{
		Vector location(radius * cos(a) + g_Local.vEye.x, radius * sin(a) + g_Local.vEye.y, g_Local.vEye.z);

		pmtrace_t tr;

		EV_SetTraceHull(2);
		EV_PlayerTrace(g_Local.vEye, location, PM_GLASS_IGNORE, -1, &tr);

		float distance = g_Local.vEye.Distance(tr.endpos);
...
void AntiAim(struct usercmd_s* cmd)
{
	if (cvar.rage_active && bAliveLocal())
	{
		...
		if (!m_Use && !IsCurWeaponNonAttack() && !(m_InAttack && CanAttack()) && !(m_InAttack2 && CanAttack()))
		{
			...
			if (target > 0) VectorAngles(origin - g_Local.vEye, vAngles);

			//Yaw
			if (g_Local.vVelocity.Length2D() > 0)
			{
