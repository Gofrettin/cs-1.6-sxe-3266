...
void TriggerTarget(struct usercmd_s* cmd, playeraim_t Aim, float& m_flBestFOV, DWORD delay, Vector vecSpreadDir)
{
	...
	for (model_aim_select_t Model_Selected : Model_Aim_Select)
	{
		...
		for (unsigned int point = 0; point < 8; ++point)
		{
			...
			if (cvar.bypass_trace_trigger)
				EV_PlayerTrace(g_Local.vEye, Aim.PlayerAimHitbox[Model_Selected.numhitbox].HitboxMulti[point], PM_WORLD_ONLY, -1, &tr);
			else
				EV_PlayerTrace(g_Local.vEye, Aim.PlayerAimHitbox[Model_Selected.numhitbox].HitboxMulti[point], PM_GLASS_IGNORE, -1, &tr);
			...
			else
			{
				...
				if (iOriginalPenetration && cvar.legit[g_Local.weapon.m_iWeaponID].trigger_wall)
				{
					...
					int iCurrentDamage = FireBullets(g_Local.vEye, Aim.PlayerAimHitbox[Model_Selected.numhitbox].HitboxMulti[point], flDistance, iOriginalPenetration, iBulletType, iDamage, flRangeModifier);
	...
	if (!hitboxselected)
	{
		...
		for (unsigned int point = 0; point < 8; ++point)
		{
			...
			if (cvar.bypass_trace_trigger)
				EV_PlayerTrace(g_Local.vEye, Aim.PlayerAimHitbox[HeadBox[Aim.ent->index]].HitboxMulti[point], PM_WORLD_ONLY, -1, &tr);
			else
				EV_PlayerTrace(g_Local.vEye, Aim.PlayerAimHitbox[HeadBox[Aim.ent->index]].HitboxMulti[point], PM_GLASS_IGNORE, -1, &tr);
			...
			else
			{
				...
				if (iOriginalPenetration && cvar.legit[g_Local.weapon.m_iWeaponID].trigger_wall)
				{
					...
					int iCurrentDamage = FireBullets(g_Local.vEye, Aim.PlayerAimHitbox[HeadBox[Aim.ent->index]].HitboxMulti[point], flDistance, iOriginalPenetration, iBulletType, iDamage, flRangeModifier);
		...
		for (unsigned int i = 0; i < 12; i++)
		{
			if (bHitboxPointsVisible[SkeletonHitboxMatrix[i][0]] && bHitboxPointsVisible[SkeletonHitboxMatrix[i][1]])
			{
				if (IsBoxIntersectingRay(Aim.PlayerAimHitbox[HeadBox[Aim.ent->index]].HitboxMulti[SkeletonHitboxMatrix[i][0]], Aim.PlayerAimHitbox[HeadBox[Aim.ent->index]].HitboxMulti[SkeletonHitboxMatrix[i][1]], g_Local.vEye, vecSpreadDir))
				{
					if (CanAttack() && GetTickCount() - delay > cvar.legit[g_Local.weapon.m_iWeaponID].trigger_delay_shot)
						cmd->buttons |= IN_ATTACK;
					break;
...
void LegitSelect(playeraim_t Aim, Vector vecFOV, float& flBestFOV, float flSpeedScaleFov, float& flSpeed)
{
	...
	if (!hitboxselected)
	{
		...
		if (cvar.bypass_trace_legit)
			EV_PlayerTrace(g_Local.vEye, Aim.PlayerAimHitbox[HeadBox[Aim.ent->index]].Hitbox, PM_WORLD_ONLY, -1, &tr);
		else
			EV_PlayerTrace(g_Local.vEye, Aim.PlayerAimHitbox[HeadBox[Aim.ent->index]].Hitbox, PM_GLASS_IGNORE, -1, &tr);

		int detect = g_Engine.pEventAPI->EV_IndexFromTrace(&tr);

		if ((cvar.bypass_trace_legit && tr.fraction == 1 && !detect) || (!cvar.bypass_trace_legit && detect == Aim.ent->index))
		{
			float fov = vecFOV.AngleBetween(Aim.PlayerAimHitbox[HeadBox[Aim.ent->index]].Hitbox - g_Local.vEye);
			if (fov < flBestFOV)
			{
				flBestFOV = fov;
				iTargetLegit = Aim.ent->index;
...
void KnifeSelect(playeraim_t Aim, float& flDist)
{
	...
	if (!hitboxselected)
	{
		...
		if ((cvar.bypass_trace_knife && tr.fraction == 1 && !detect) || (!cvar.bypass_trace_knife && detect == Aim.ent->index))
		{
			...
			{
				if (Aim.PlayerAimHitbox[HeadBox[Aim.ent->index]].Hitbox.Distance(g_Local.vEye) < flDist)
				{
					flDist = Aim.PlayerAimHitbox[HeadBox[Aim.ent->index]].Hitbox.Distance(g_Local.vEye);
					iTargetKnife = Aim.ent->index;
...
void KnifeAimBot(struct usercmd_s* cmd)
{
	...
	if (iTargetKnife)
	{
		...
		if (vAimOriginKnife.Distance(g_Local.vEye) < dist)
		{
			if (CanAttack())
			{
				QAngle QAimAngles;
...
void RageTarget(playeraim_t Aim, float& m_flBestDist, float& m_flBestFOV, int hitbox)
{
	if (IsCurWeaponKnife())
	{
		if (Aim.PlayerAimHitbox[hitbox].Hitbox.Distance(g_Local.vEye) < m_flBestDist)
		{
			m_flBestDist = Aim.PlayerAimHitbox[hitbox].Hitbox.Distance(g_Local.vEye);
	...
	for (model_aim_select_t Model_Selected : Model_Aim_Select)
	{
		...
		else
		{
			...
			if (iOriginalPenetration && cvar.rage_wall)
			{
				...
				int iCurrentDamage = FireBullets(g_Local.vEye, Aim.PlayerAimHitbox[Model_Selected.numhitbox].Hitbox, flDistance, iOriginalPenetration, iBulletType, iDamage, flRangeModifier);
	...
	if (!hitboxselected)
	{
		...
		else
		{
			...
			if (iOriginalPenetration && cvar.rage_wall)
			{
				...
				int iCurrentDamage = FireBullets(g_Local.vEye, Aim.PlayerAimHitbox[HeadBox[Aim.ent->index]].Hitbox, flDistance, iOriginalPenetration, iBulletType, iDamage, flRangeModifier);
	...
	if (iTargetKnife)
	{
		for (playeraim_t Aim : PlayerAim)
		{
			...
			for (unsigned int i = 0; i < 12; i++)
			{
				...
				if (WorldScreen(Aim.PlayerAimHitbox[iHitboxKnife].HitboxMulti[SkeletonHitboxMatrix[i][0]], CalcAnglesMin) && WorldScreen(Aim.PlayerAimHitbox[iHitboxKnife].HitboxMulti[SkeletonHitboxMatrix[i][1]], CalcAnglesMax))
...
void RageDraw()
{
	...
	if (iTargetRage)
	{
		for (playeraim_t Aim : PlayerAim)
		{
			...
			{
				float CalcAnglesMin[2], CalcAnglesMax[2];
				if (WorldScreen(Aim.PlayerAimHitbox[iHitboxRage].HitboxMulti[SkeletonHitboxMatrix[i][0]], CalcAnglesMin) && WorldScreen(Aim.PlayerAimHitbox[iHitboxRage].HitboxMulti[SkeletonHitboxMatrix[i][1]], CalcAnglesMax))
					ImGui::GetCurrentWindow()->DrawList->AddLine({ IM_ROUND(CalcAnglesMin[0]), IM_ROUND(CalcAnglesMin[1]) }, { IM_ROUND(CalcAnglesMax[0]), IM_ROUND(CalcAnglesMax[1]) }, Green());
			}
		}
	}
...
void LegitDraw()
{
	...
	if (iTargetLegit)
	{
		for (playeraim_t Aim : PlayerAim)
		{
			...
			{
				float CalcAnglesMin[2], CalcAnglesMax[2];
				if (WorldScreen(Aim.PlayerAimHitbox[iHitboxLegit].HitboxMulti[SkeletonHitboxMatrix[i][0]], CalcAnglesMin) && WorldScreen(Aim.PlayerAimHitbox[iHitboxLegit].HitboxMulti[SkeletonHitboxMatrix[i][1]], CalcAnglesMax))
					ImGui::GetCurrentWindow()->DrawList->AddLine({ IM_ROUND(CalcAnglesMin[0]), IM_ROUND(CalcAnglesMin[1]) }, { IM_ROUND(CalcAnglesMax[0]), IM_ROUND(CalcAnglesMax[1]) }, Green());
			}
		}
	}
...
void TriggerDrawTarget(playeraim_t Aim)
{
	...
	for (model_aim_select_t Model_Selected : Model_Aim_Select)
	{
		...
		for (unsigned int i = 0; i < 12; i++)
		{
			...
			{
				float CalcAnglesMin[2], CalcAnglesMax[2];
				if (WorldScreen(Aim.PlayerAimHitbox[Model_Selected.numhitbox].HitboxMulti[SkeletonHitboxMatrix[i][0]], CalcAnglesMin) && WorldScreen(Aim.PlayerAimHitbox[Model_Selected.numhitbox].HitboxMulti[SkeletonHitboxMatrix[i][1]], CalcAnglesMax))
					ImGui::GetCurrentWindow()->DrawList->AddLine({ IM_ROUND(CalcAnglesMin[0]), IM_ROUND(CalcAnglesMin[1]) }, { IM_ROUND(CalcAnglesMax[0]), IM_ROUND(CalcAnglesMax[1]) }, Green());
			}
		}
	}
	if (!hitboxselected)
	{
		...
		for (unsigned int i = 0; i < 12; i++)
		{
			...
			{
				float CalcAnglesMin[2], CalcAnglesMax[2];
				if (WorldScreen(Aim.PlayerAimHitbox[HeadBox[Aim.ent->index]].HitboxMulti[SkeletonHitboxMatrix[i][0]], CalcAnglesMin) && WorldScreen(Aim.PlayerAimHitbox[HeadBox[Aim.ent->index]].HitboxMulti[SkeletonHitboxMatrix[i][1]], CalcAnglesMax))
					ImGui::GetCurrentWindow()->DrawList->AddLine({ IM_ROUND(CalcAnglesMin[0]), IM_ROUND(CalcAnglesMin[1]) }, { IM_ROUND(CalcAnglesMax[0]), IM_ROUND(CalcAnglesMax[1]) }, Green());
			}
		}
	}
