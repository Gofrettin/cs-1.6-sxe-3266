...
void MenuLegit1()
{
	...
	if (CheckWeapon(cvar.menu_legit_global_section, cvar.menu_legit_sub_section))
	{
		...
		ImGui::Text("Silent Attack Speed");
		SliderFloat("Silent Attack Speed##1", &cvar.legit[CheckWeapon(cvar.menu_legit_global_section, cvar.menu_legit_sub_section)].silent, 0.f, 100.f, "%.0f");
...
void MenuIdHook1()
{
	...
	for (unsigned int id = 1; id <= g_Engine.GetMaxClients(); id++)
	{
		...
		else
		{
			...
			ImGui::PushStyleColor(ImGuiCol_Text, col);
			char str[256];
			sprintf(str, "%s [Off]", entinfo.name);
			if (ImGui::Button(str))
				idhook.AddPlayer(id);
			if (g_Player[id].bVip) ImGui::SameLine(), ImGui::Image((GLuint*)texture_id[VIP], ImVec2(23, 17));
			ImGui::PopStyleColor();
...
void MenuVisual4Window1()
{
	if (cvar.visual_skins_wall)
	{
		...
		if (ImGui::Button("Load"))
			loadwall = true;
		cl_entity_s* ent = GetEntityByIndex(0);
...
void DrawMenuChild(int total)
{
	...
	if (showspeed < menusize)
	{
		...
		if (MenuTab != 4 && MenuTab != 11 && MenuTab != 13 && MenuTab != 14)
		{
			...
			ImGui::SetNextWindowSize(ImVec2(0, windowheight2));
