...
void DrawOverviewLayer()
{
	...
	if (m_MapSprites) 
	{
		...
		float z = ((90.0f - g_Local.vAngles[0]) / 90.0f) * m_OverviewData.layersHeights[0];
		float xStep = (8192.f / cvar.radar_zoom) / xTile;
		float yStep = -(8192.f / (cvar.radar_zoom * (4.f / 3.f))) / yTile;
		float angle = (float)((g_Local.vAngles[1] + 90.0) * (M_PI / 180));
		...
		if (m_OverviewData.rotated) 
		{
			...
			tile_y = -(float)(origin_tilex - (1.0 / 1024) * m_OverviewData.zoom * g_Local.vEye[0]);
			tile_x = (float)(origin_tiley - (1.0 / 1024) * m_OverviewData.zoom * g_Local.vEye[1]);
		}
		else 
		{ 
			...
			tile_x = (float)(origin_tilex - (1.0 / 1024) * m_OverviewData.zoom * g_Local.vEye[0]);
			tile_y = (float)(origin_tiley - (1.0 / 1024) * m_OverviewData.zoom * g_Local.vEye[1]);
		}
...
void DrawOverviewEntities()
{
	...
	for (unsigned int i = 1; i <= g_Engine.GetMaxClients(); i++)
	{
		...
		if (!ent || !ent->player)
			continue;
		if (ent->curstate.messagenum < GetEntityByIndex(g_Local.iPlayer_Index + 1)->curstate.messagenum)
			continue;
		...
		if (ent->curstate.maxs.IsZero())
			continue;
		if (GetEntityByIndex(g_Local.iPlayer_Index + 1)->curstate.iuser2 == ent->index)
			continue;
		...
		float calcscreen[2];
		VectorRotateZ(ent->origin - g_Local.vEye, -g_Local.vAngles[1], calcscreen);
		...
		Vector vforward, vRight, vAngles = Vector(0, -ent->angles[1] + g_Local.vAngles[1] + 90, 0);
...
void DrawOverviewEntitiesSoundIndex()
{
	...
	for (player_sound_index_t sound_index : Sound_Index)
	{
		if (GetTickCount() - sound_index.timestamp > 300)
			continue;
		cl_entity_s* ent = GetEntityByIndex(sound_index.index);
		if (!ent)
			continue;
		...
		float calcscreen[2];
		VectorRotateZ(sound_index.origin - g_Local.vEye, -g_Local.vAngles[1], calcscreen);
		...
		QAngle QAimAngles;
		VectorAngles(sound_index.origin - g_Local.vEye, QAimAngles);
		Vector vforward, vRight, vAngles = Vector(0, -QAimAngles[1] + g_Local.vAngles[1] - 90, 0);
		g_Engine.pfnAngleVectors(vAngles, vforward, vRight, NULL);
...
void DrawOverviewEntitiesSoundNoIndex()
{
	...
	for (player_sound_no_index_t sound_no_index : Sound_No_Index)
	{
		if (GetTickCount() - sound_no_index.timestamp > 300)
			continue;
		float calcscreen[2];
		VectorRotateZ(sound_no_index.origin - g_Local.vEye, -g_Local.vAngles[1], calcscreen);
		float screenx = IM_ROUND(iX + iW / 2 - calcscreen[1] / cvar.radar_zoom * m_OverviewData.zoom * 0.3f * iW / 2 / (ImGui::GetIO().DisplaySize.x / 6.7));
		float screeny = IM_ROUND(iY + iH / 2 - calcscreen[0] / cvar.radar_zoom * m_OverviewData.zoom * 0.4f * iH / 2 / (ImGui::GetIO().DisplaySize.y / 5.1));
		...
		QAngle QAimAngles;
		VectorAngles(sound_no_index.origin - g_Local.vEye, QAimAngles);
		Vector vforward, vRight, vAngles = Vector(0, -QAimAngles[1] + g_Local.vAngles[1] - 90, 0);
		g_Engine.pfnAngleVectors(vAngles, vforward, vRight, NULL);
