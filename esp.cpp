...
bool Name(int id, float x, float y, ImU32 team, ImU32 white)
{
	...
	pfnGetPlayerInfo(id, &entinfo);
	if (!entinfo.name) return false;
	float label_size = IM_ROUND(ImGui::CalcTextSize(entinfo.name, NULL, true).x / 2);
	ImGui::GetCurrentWindow()->DrawList->AddRect({ x - label_size - 2, y - 15 }, { x + label_size + 3 , y - 1 }, team);
	ImGui::GetCurrentWindow()->DrawList->AddText({ x - label_size, y - 16 }, white, entinfo.name);
	return true;
}
...
bool Model(int id, float x, float y, ImU32 team, ImU32 white)
{
	...
	hud_player_info_t entinfo;
	pfnGetPlayerInfo(id, &entinfo);
	if (!entinfo.model) return false;
	float label_size = IM_ROUND(ImGui::CalcTextSize(entinfo.model, NULL, true).x / 2);
	ImGui::GetCurrentWindow()->DrawList->AddRect({ x - label_size - 2, y - 15 }, { x + label_size + 3 , y - 1 }, team);
	ImGui::GetCurrentWindow()->DrawList->AddText({ x - label_size, y - 16 }, white, entinfo.model);
	return true;
}

bool Weapon(cl_entity_s* ent, float x, float y, ImU32 team, ImU32 white)
{
	model_s* mdl = GetModelByIndex(ent->curstate.weaponmodel);
...
{
	float vOrigin[2];
	if (!WorldScreen(Esp.ent->origin, vOrigin)) return false;
	...
	for (playeresphitbox_t Hitbox : Esp.PlayerEspHitbox)
	{
		...
		{
			float vHitbox[2];
			if (!WorldScreen(Hitbox.HitboxMulti[i], vHitbox)) return false;
			x0 = min(x0, vHitbox[0]);
			x1 = max(x1, vHitbox[0]);
			y0 = min(y0, vHitbox[1]);
			y1 = max(y1, vHitbox[1]);
	...
	for (player_sound_index_t sound_index : Sound_Index)
	{
		...
		if (cvar.visual_sound_steps)
		{
			...
			for (float i = 0; i < (IM_PI * 2.0f); i += step)
			{
				...
				if (WorldScreen(vPointStart, vStart) && WorldScreen(vPointEnd, vEnd))
		...
		float vTop[2], vBot[2];
		if (WorldScreen(vPointTop, vTop) && WorldScreen(vPointBot, vBot))
		{
