...
void TraceGrenade(ref_params_s* pparams)
{
	if (cvar.visual_grenade_trajectory && IsCurWeaponNade() && bAliveLocal())
	{
		...
		float flGravity = g_Local.flMoveGravity / SVGRAVITY;
		...
		for (flTimeAlive = 0; flTimeAlive < 3.00f; flTimeAlive += flStep)
		{
			...
			if (pmtrace.ent != g_Local.iPlayer_Index + 1 && pmtrace.fraction < 1.0)
			{
...
void DrawTraceGrenade()
{
	for (grenadeline_t Grenade : Grenadeline)
	{
		...
		if (WorldScreen(Grenade.GrenadePos1, VecScreenMin) && WorldScreen(Grenade.GrenadePos2, VecScreenMax))
		...
		if (WorldScreen(Grenade.GrenadePos2, VecScreenMax))
