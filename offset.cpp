...
ULONG __findmemoryclones(const ULONG start, const ULONG end, const ULONG clone, UINT size)
{
	for (ULONG ul = start; (ul + size) < end; ul++)
	{
...
#define FindMemoryClone(Start, End, Clone, Size) __findmemoryclones((const ULONG)Start, (const ULONG)End, (const ULONG)Clone, (UINT)Size)
...
{
	DWORD UiBase = (DWORD)GetModuleHandle("GameUI.dll"); 
	DWORD UiSize = (DWORD)GetModuleSize(UiBase);
	DWORD UiEnd = UiBase + UiSize - 1;
...
{
	static DWORD HwBase = (DWORD)GetModuleHandleA(NULL);
	static DWORD HwSize = 0x02116000;
	static DWORD HwEnd = HwBase + HwSize - 1;
