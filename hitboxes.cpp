...
void GetHitboxes(cl_entity_s* ent)
{
	if (ent && ent->model && ent->model->name)
	{
		...
		if (!strstr(ent->model->name, "player") && ent->index != g_Local.iPlayer_Index + 1)
		{
		...
		if (strstr(ent->model->name, "v_") && ent->index == g_Local.iPlayer_Index + 1)
		{
			...
			for (unsigned int i = 0; i < pStudioHeader->numhitboxes; i++)
			{
				...
				if (!IsSHield(vCubePointsTrans))
				{
					...
					for (unsigned int x = 0; x < 8; x++)
					{
						...
						AimHitbox.HitboxPointsFOV[x] = g_Local.vPrevForward.AngleBetween(AimHitbox.HitboxMulti[x] - g_Local.vEye);
					}
					AimHitbox.Hitbox = (vBBMax + vBBMin) * 0.5f;
					AimHitbox.HitboxFOV = g_Local.vPrevForward.AngleBetween(AimHitbox.Hitbox - g_Local.vEye);
...
void DrawSkeletonPlayer()
{
	for (playerbone_t Bones : PlayerBone)
	{
		...
		if (WorldScreen(Bones.vBone, CalcAnglesMin) && WorldScreen(Bones.vBoneParent, CalcAnglesMax))
	...
	for (playerhitbox_t Hitbox : PlayerHitbox)
	{
		...
		{
			float CalcAnglesMin[2], CalcAnglesMax[2];
			if (WorldScreen(Hitbox.vCubePointsTrans[SkeletonHitboxMatrix[x][0]], CalcAnglesMin) && WorldScreen(Hitbox.vCubePointsTrans[SkeletonHitboxMatrix[x][1]], CalcAnglesMax))
				ImGui::GetCurrentWindow()->DrawList->AddLine({ IM_ROUND(CalcAnglesMin[0]), IM_ROUND(CalcAnglesMin[1]) }, { IM_ROUND(CalcAnglesMax[0]), IM_ROUND(CalcAnglesMax[1]) }, Player);
		}
	}
	for (playerhitboxnum_t HitboxNum : PlayerHitboxNum)
	{
		...
		if (WorldScreen(HitboxNum.HitboxPos, CalcAnglesMin))
		{
			char str[256];
...
void DrawSkeletonWorld()
{
	for (worldbone_t Bones : WorldBone)
	{
		if (Bones.parent >= 0)
		{
			...
			if (Bones.parent != -1)
			{
				if (WorldScreen(Bones.vBoneParent, CalcAnglesMin))
			...
			if (WorldScreen(Bones.vBone, CalcAnglesMin))
				ImGui::GetCurrentWindow()->DrawList->AddRectFilled({ IM_ROUND(CalcAnglesMin[0]) - 1, IM_ROUND(CalcAnglesMin[1]) - 1 }, { IM_ROUND(CalcAnglesMin[0]) + 2, IM_ROUND(CalcAnglesMin[1]) + 2 }, Wheel1());
		}
		else
		{
	...
	for (worldhitbox_t Hitbox : WorldHitbox)
	{
		...
		{
			float CalcAnglesMin[2], CalcAnglesMax[2];
			if (WorldScreen(Hitbox.vCubePointsTrans[SkeletonHitboxMatrix[i][0]], CalcAnglesMin) && WorldScreen(Hitbox.vCubePointsTrans[SkeletonHitboxMatrix[i][1]], CalcAnglesMax))
		...
		{
			float CalcAnglesMin[2];
			if (WorldScreen(Hitbox.vCubePointsTrans[i], CalcAnglesMin))
