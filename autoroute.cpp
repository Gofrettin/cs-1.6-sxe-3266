...
{
	return g_Local.vEye.Distance(pos);
}
...
void AutoRoute::AddPoint()
{
	if (iRouteLine > -1)
	{
		...
		route_line[iRouteLine].point[iRoutePoint].origin = g_Local.vEye;
...
void DrawRouteLine()
{
	for (route_t Route : Routeline)
	{
		if (Route.Draw1)
		{
			...
			if (WorldScreen(Route.RoutePos1, vecScreenMin) && WorldScreen(Route.RoutePos2, vecScreenMax))
		...
		if (Route.Draw2)
		{
			...
			if (WorldScreen(Route.RoutePos3, vecScreenMin))
			{
